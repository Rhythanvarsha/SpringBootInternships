...
//      assertEquals(3,result.size());   //test fails
    ...
    void testGetIdMethod(){
        ...
        when(registerDetailsRepository.findById(1)).thenReturn(empl1);//when is present inside mokito//this line do (when we call for registerDetailsRepository.findAll() by usinggetAllEmployeeees in service then return the empl1 and empl2 as list
        RegisterDetails result=employeeService.getEmployeeById(1);//if we findall the employee then return the above empl1,empl2 as list
        System.out.println(result);
        assertEquals("null",result);//as result has 2 list empl1 and empl2
//      assertEquals(3,result.size());   //test fails
    }
}
